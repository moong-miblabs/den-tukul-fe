...
async function getDATA() {
    ...
    var response = await $axios.get('deteksi')
    $.LoadingOverlay("hide");
    ...
    if(response.data.status === "success") {
        ...
        DATA.value = data.map(obj => {
            ...
            obj.klasifikasi = obj.klasifikasi=="P"?"Positif":"Negatif"
            return obj
        })
    }
}
...
<template>
    <div class="card">
        <div class="font-semibold text-xl mb-4">Hasil Deteksi</div>
        <DataTable :value="DATA" tableStyle="min-width: 50rem">
            ...
            <Column header="Detail">
                <template #body="slotProps">
                    <Button @click="$router.push('/result/'+slotProps.data.id)" label="Detail" icon="pi pi-info-circle"></Button>
                </template>
            </Column>
        </DataTable>
    </div>
